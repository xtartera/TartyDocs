# Proposta · Comparativa pràctica d'operacions: Samba vs NFS

> Generat per Claude arran d'una observació de l'usuari en revisar els continguts publicats de la UT3.

## Diagnosi

Revisant els blocs 2–6 de la UT3, els apunts cobreixen amb detall la **configuració** de Samba i NFS (`smb.conf`, `/etc/exports`, control d'accés, seguretat...), però en cap pàgina es tracta explícitament la diferència **operativa** entre tots dos un cop l'usuari ja hi és connectat:

- **Samba via `smbclient`** obre un shell interactiu de tipus FTP amb un joc d'ordres tancat (`ls`, `get`, `put`, `cd`, `mkdir`, `del`...). No es pot editar un fitxer remot in situ, ni aplicar-hi `grep`, `sed`, `tar` o `rsync` directament — cal baixar-lo, treballar-hi localment i tornar-lo a pujar.
- **Samba via `mount -t cifs`** (Bloc 6, pàgina 22) sí que munta un sistema de fitxers, però amb el muntatge estàndard que s'ensenya (`uid=`, `gid=`, `file_mode=`, `dir_mode=`) els permisos i el propietari són **fixos per a tot el recurs**: `chmod`/`chown` sobre un fitxer concret no tenen efecte real, i els enllaços simbòlics (`ln -s`) normalment fallen.
- **NFS**, en canvi, és nadiu de Unix: un cop muntat es comporta exactament com un sistema de fitxers local — `chmod`, `chown` (amb els límits de `squash` ja explicats), `ln -s`, `ln` (hard links) i qualsevol ordre de Linux funcionen sense cap configuració addicional.

Aquest contrast és precisament el que fa que, a la pràctica professional, es percebi Samba com "més feixuc" i NFS com amb "més llibertat de moviments" — però la raó tècnica de fons (interfície de transferència vs sistema de fitxers Unix natiu) no queda mai explicada.

## Recomanació

**Sí, val la pena afegir-ho.** No és un detall menor: és exactament el tipus de coneixement que distingeix algú que sap *configurar* Samba/NFS d'algú que sap *triar i argumentar* quina tecnologia usar per a cada tasca — una competència que encaixa amb els RA d'aquest mòdul (RA3–RA5) i que reforça la comparativa que ja es fa al Bloc 1 (protocols) des d'un angle pràctic en lloc de purament teòric.

### On situar-ho

Proposo una **pàgina nova al final del Bloc 6** ("NFS i client SMB"), ja que és el bloc on conviuen els dos client-side (`mount -t nfs` i `smbclient`/`mount -t cifs`) i on l'alumnat ja ha practicat totes dues connexions. Encaixaria com a pàgina `22b` (o renumerant-la com a `23` i desplaçant CUPS una posició, segons prefereixis mantenir la numeració actual).

Alternativa més lleugera: ampliar la pàgina existent [22-smbclient-mount-cifs.md](../docs/ut3/bloc6-nfs-client-seguretat/22-smbclient-mount-cifs.md) amb una secció nova en comptes de crear un fitxer independent. Ho deixo com a pàgina separada perquè el contingut ja té prou entitat pròpia (activitat, auto-avaluació) i no sobrecarrega una pàgina que ja és densa.

---

## Contingut proposat (llest per copiar a `docs/ut3/bloc6-nfs-client-seguretat/`)
...
- El fitxer anterior segueix exactament l'estructura de la resta de pàgines de bloc (`abstract`, pestanyes `Apunts`/`Activitat`/`Vídeo`, `warning`, `auto-avaluació` amb `??? question`), de manera que es pot enganxar directament dins `docs/ut3/bloc6-nfs-client-seguretat/` sense ajustos d'estil.
- Si s'incorpora, caldria:
